...
EBTNodeResult::Type UBTTargetPointSelection::ExecuteTask(UBehaviorTreeComponent & OwnerComp, uint8 * NodeMemory)
{
	...
	if (AICon)
	{
		...
		APatrolTargetPoint* CurrentPoint = Cast<APatrolTargetPoint>(BlackboardComp->GetValueAsObject("LocationToGo"));
		...
		APatrolTargetPoint* NextTargetPoint = nullptr;
		...
		do
		{
			...
			NextTargetPoint = Cast<APatrolTargetPoint>(AvailableTargetPoints[RandomIndex]);
